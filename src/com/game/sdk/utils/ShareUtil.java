...
public class ShareUtil {
	...
	public static void openWXShareWithImage(final Context ctx, final String content, final List<String> images,
		...
		if (!CheckUtil.isWeixinAvilible(ctx)) {
			Util.toast(ctx, "请安装微信");
			return;
		}

		final CustomDialog shareDialog = new CustomDialog(ctx, "正在分享");
		shareDialog.show();
	...
	public static void openWXShareWithImage(final Context ctx, final String content) {
		if (!CheckUtil.isWeixinAvilible(ctx)) {
			Util.toast(ctx, "请安装微信");
			return;
		}
		...
		if (GoagalInfo.inItInfo != null && GoagalInfo.inItInfo.logoBitmp != null) {
			bmp = GoagalInfo.inItInfo.logoBitmp;
		} else {
			bmp = BitmapFactory.decodeResource(ctx.getResources(),
					MResource.getIdByName(ctx, "drawable", "quick_title_icon"));
		}
