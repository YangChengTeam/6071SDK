...
public class MobileInfoUtil {
	...
	public static List<UserInfo> loadAllUserInfo(Context context) {
		...
		if (!userInfosStr.isEmpty()) {
			...
			try {
				...
				/*
				 * JSONArray jsonArray = new JSONArray(userInfosStr); for (int i
				 * = 0; i < jsonArray.length(); i++) { JSONObject jsonObject =
				 * jsonArray.getJSONObject(i); UserInfo userInfo = new
				 * UserInfo();
				 * 
				 * userInfo.accountType = jsonObject.getInt("accountType") > -1
				 * ? jsonObject.getInt("accountType") : 0; userInfo.userId =
				 * jsonObject.getString("user_id") != null ?
				 * jsonObject.getString("user_id") :""; userInfo.username =
				 * jsonObject.getString("name")!= null ?
				 * jsonObject.getString("name") :""; userInfo.password =
				 * jsonObject.getString("password")!= null ?
				 * jsonObject.getString("password") :""; userInfo.device =
				 * jsonObject.getInt("device") > -1 ?
				 * jsonObject.getInt("device") : -1; userInfo.validateMobile =
				 * jsonObject.getInt("is_vali_mobile") > -1 ?
				 * jsonObject.getInt("is_vali_mobile") : -1; userInfo.isrpwd =
				 * jsonObject.getInt("isrpwd") > -1 ?
				 * jsonObject.getInt("isrpwd") : -1; userInfo.logintime =
				 * jsonObject.getLong("logintime") > -1 ?
				 * jsonObject.getLong("logintime") : 0 ; userInfo.sex =
				 * jsonObject.getInt("sex") > -1 ? jsonObject.getInt("sex") :
				 * -1; userInfo.sign = jsonObject.getString("sign")!= null ?
	...
	private static File getUserInfosFile(Context context) {
		return new File(getFilePath(md5(getUid(context) + "mobiles")));
	}
