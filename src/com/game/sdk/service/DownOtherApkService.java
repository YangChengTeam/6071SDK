...
public class DownOtherApkService extends Service {
	...
	private void createNotification() {
		...
		mBuilder.setContentTitle("开始下载游戏应用").setWhen(System.currentTimeMillis())
		...
		if (intent.getStringExtra("downUrl") != null && intent.getStringExtra("downUrl").length() > 0) {
			...
			mOkHttpClient.newCall(request).enqueue(new okhttp3.Callback() {
				...
				public void onResponse(Call arg0, okhttp3.Response response) throws IOException {
					...
					String apkPath = PathUtil.getApkPath("other_game");
					try {
						...
						if(file.exists()){
							if(ZipUtil.isArchiveFile(file)){
								...
								Util.toast(DownOtherApkService.this, "游戏应用文件错误，请重新下载");
							}
						}
						
					} catch (Exception e) {
						Logger.msg("游戏应用下载失败");
						mBuilder.setContentTitle("游戏应用下载失败").setProgress(0, 0, true);
						nm.notify(notifyId, mBuilder.build());
					} finally {
						try {
							if (is != null)
			...
			handler.post(new Runnable() {
				@Override
				public void run() {
					Toast.makeText(DownOtherApkService.this, "下载地址错误，请稍后重试", Toast.LENGTH_SHORT).show();
				}
			});
		}
		return 1;
