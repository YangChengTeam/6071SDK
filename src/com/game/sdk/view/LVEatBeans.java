...
public class LVEatBeans extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		int scale = DimensionUtil.photoDensity(getContext());
		int tempNum = 10;
		if(scale > 2){
			tempNum = (scale - 1) * 5;
		}else{
		...
		int beansCount = (int) ((mWidth - mPadding * 2 - eatErWidth) / beansWidth) - tempNum;
		// Log.e("beansCount----", "beansCount---" + beansCount);
		for (int i = 0; i < beansCount; i++) {

			float x = beansCount * i * 3 + beansWidth / 2 + mPadding + eatErWidth;
			// Log.e("x----", "x---" + x);
			if (x > eatRightX) {
	...
	private void initPaint() {
		...
		mPaint.setColor(
				getContext().getResources().getColor(MResource.getIdByName(getContext(), "color", "loading_color")));

		mPaintEye = new Paint();
		mPaintEye.setAntiAlias(true);
		mPaintEye.setStyle(Paint.Style.FILL);
		mPaintEye.setColor(
				getContext().getResources().getColor(MResource.getIdByName(getContext(), "color", "loading_circle")));
	...
	private ValueAnimator startViewAnim(float startF, final float endF, long time) {
		...
		valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator valueAnimator) {
				...
				float mAnimatedValue = Float.valueOf(valueAnimator.getAnimatedValue() + "");
